...
export class AdminUsersController {
  ...
  async create(@Body() body: {
    ...
    if (!['student', 'teacher'].includes(role)) {
      throw new BadRequestException('role must be student|teacher');
    }
  ...
  async updateNames(
    ...
    const user = await this.prisma.user.update({
      where: { id },
      data: { firstName: body.firstName ?? null, lastName: body.lastName ?? null },
  ...
  async remove(@Param('id') id: string) {
    ...
    if (!user) throw new NotFoundException();
    if (user.role === 'admin') throw new ForbiddenException('cannot delete admin');
