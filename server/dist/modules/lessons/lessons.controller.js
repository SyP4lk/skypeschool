...
let LessonsController = class LessonsController {
    ...
    async update(req, id, data) {
        ...
        const user = req.user;
        if (!ls)
            throw new (await Promise.resolve().then(() => __importStar(require('@nestjs/common')))).BadRequestException('lesson not found');
        if (!(user?.role === 'admin' || (user?.role === 'teacher' && ls.teacherId === user.sub))) {
            throw new (await Promise.resolve().then(() => __importStar(require('@nestjs/common')))).ForbiddenException();
        }
        const allowed = {};
