...
let AuthService = class AuthService {
    ...
    async validateUser(ident, password) {
        const id = (ident || '').trim().toLowerCase();
        let user = await this.tryFindByLogin(id);
        if (!user && id.includes('@'))
            user = await this.tryFindByEmail(id);
        if (!user) {
            const phone = (0, phone_util_1.normalizePhone)(ident);
            if (phone)
                user = await this.tryFindByPhone(phone);
        }
        if (!user)
